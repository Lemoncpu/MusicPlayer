...
const LoginPage = {
  setup() {
    ...
    return { state, mode, form, busy, submit };
  },
  template: `
    <div class="auth-layout"><div class="auth-panel"><section class="auth-hero"><div><span class="eyebrow">{{ $root.T.app }}</span><h1>{{ $root.T.loginTitle }}</h1><p>{{ mode === 'login' ? $root.T.loginText : $root.T.registerText }}</p></div><div class="auth-badges"><span>{{ $root.T.library }}</span><span>{{ $root.T.importLocal }}</span><span>{{ $root.T.playlists }}</span></div></section><section class="auth-card"><div class="auth-tabs"><button type="button" :class="{ active: mode === 'login' }" @click="mode = 'login'">{{ $root.T.login }}</button><button type="button" :class="{ active: mode === 'register' }" @click="mode = 'register'">{{ $root.T.login === '登录' ? '注册' : 'Register' }}</button></div><input v-model.trim="form.username" :placeholder="$root.T.username"><input v-model="form.password" type="password" :placeholder="$root.T.password"><button class="primary-button wide" type="button" :disabled="busy" @click="submit">{{ mode === 'login' ? $root.T.login : ($root.T.login === '登录' ? '注册' : 'Register') }}</button><p class="status-text">{{ state.statusMessage }}</p></section></div></div>
  `
};
